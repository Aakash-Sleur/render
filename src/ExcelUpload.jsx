...
const ExcelUploader = () => {
  ...
  const parseExcelFile = async () => {
    ...
    try {
      ...
      setStatus("success");
      setMessage(
        `Successfully parsed ${formattedData.length} questions from the Excel file`
      );
    } catch (error) {
      setStatus("error");
      ...
      const processedData = parsedData.map((row, index) => {
        ...
        if (qType !== "Numerical type") {
          ...
          if (row["optionB"] || row["optionB"])
            options.push({ text: row["optionB"] || row["optionB"] });
          if (row["optionC"] || row["optionC"])
            options.push({ text: row["optionC"] || row["optionC"] });
          if (row["optionD"] || row["optionD"])
            options.push({ text: row["optionD"] || row["optionD"] });
        }
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">
          Excel Question Uploader
        </h1>
        <p className="text-gray-600">
          Upload Excel files with question data to your server
        </p>
      </div>
          ...
          <div className="bg-white rounded-lg border overflow-hidden">
                    ...
                    return (
                      <tr
                        ...
                        className={`border-t hover:bg-gray-50 ${
                          !completeness.isComplete ? "bg-red-50" : ""
                        }`}
                      >
                        ...
                        <td className="px-3 py-2">
                          {completeness.isComplete ? (
                            ...
                            <span
                              ...
                              title={getMissingFieldsText(item)}
                            >
            ...
            {parsedData.length > 5 && (
              <div className="px-4 py-3 bg-gray-50 text-sm text-gray-600 text-center border-t">
                Showing 5 of {parsedData.length} questions. Click "View All
                Data" to see everything.
              </div>
            )}
          </div>
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            <div className="bg-white p-3 rounded-lg border">
              <div className="text-2xl font-bold text-green-600">
                {
                  parsedData.filter((q) => getRowCompleteness(q).isComplete)
                    .length
                }
              </div>
          ...
          <div className="bg-white rounded-lg max-w-[95vw] max-h-[90vh] overflow-hidden w-full">
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-100 sticky top-0">
                  <tr>
                    ...
                    <th className="px-3 py-3 text-left font-semibold min-w-80">
                      Question
                    </th>
                    <th className="px-3 py-3 text-left font-semibold min-w-40">
                      Option A
                    </th>
                    <th className="px-3 py-3 text-left font-semibold min-w-40">
                      Option B
            ...
            <div className="flex justify-between items-center p-4 border-t bg-gray-50">
              <div className="text-sm text-gray-600">
                Total: {parsedData.length} questions | Complete:{" "}
                {
                  parsedData.filter((q) => getRowCompleteness(q).isComplete)
                    .length
                }{" "}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-blue-50 p-4 rounded-lg">
                  ...
                  <p className="text-blue-800">
                    {selectedQuestion.optionA || "Not provided"}
                  </p>
                </div>
                <div className="bg-green-50 p-4 rounded-lg">
                  ...
                  <p className="text-green-800">
                    {selectedQuestion.optionB || "Not provided"}
                  </p>
                </div>
                <div className="bg-yellow-50 p-4 rounded-lg">
                  ...
                  <p className="text-yellow-800">
                    {selectedQuestion.optionC || "Not provided"}
                  </p>
                </div>
                <div className="bg-purple-50 p-4 rounded-lg">
                  ...
                  <p className="text-purple-800">
                    {selectedQuestion.optionD || "Not provided"}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-red-50 p-4 rounded-lg">
                ...
                <p className="text-red-800 text-lg font-bold">
                  {selectedQuestion.answers || "Not provided"}
                </p>
              </div>
                ...
                <div className="space-y-3">
                  <div className="bg-gray-50 p-3 rounded">
                    ...
                    <p className="text-gray-600 mt-1">
                      {selectedQuestion.qnType || "Not specified"}
                    </p>
                  </div>
                  <div className="bg-gray-50 p-3 rounded">
                    <span className="font-medium text-gray-700">Topic:</span>
                    <p className="text-gray-600 mt-1">
                      {selectedQuestion.topic || "Not specified"}
                    </p>
                  </div>
                  <div className="bg-gray-50 p-3 rounded">
                    <span className="font-medium text-gray-700">Chapter:</span>
                    <p className="text-gray-600 mt-1">
                      {selectedQuestion.chapter || "Not specified"}
                    </p>
                  </div>
                </div>
                <div className="space-y-3">
                  <div className="bg-gray-50 p-3 rounded">
                    ...
                    <p className="text-gray-600 mt-1">
                      {selectedQuestion.subject || "Not specified"}
                    </p>
                  </div>
                  <div className="bg-gray-50 p-3 rounded">
                    <span className="font-medium text-gray-700">Exam:</span>
                    <p className="text-gray-600 mt-1">
                      {selectedQuestion.exam || "Not specified"}
                    </p>
                  </div>
                </div>
              </div>
      ...
      <div className="mt-8 bg-blue-50 rounded-lg p-4">
        ...
        <div className="text-sm text-blue-700 space-y-1">
          <p>
            <strong>Required Columns:</strong> Serial Number, Question, Option
            A/a, Option B/b, Option C/c, Option D/d
          </p>
          <p>
            <strong>Optional Columns:</strong> Answers/Correct option, Question
            Type, Topic, Chapter, Subject, Exam
          </p>
          <p>
            <strong>Note:</strong> Column names are case-insensitive for options
            (e.g., 'Option A' or 'Option a')
          </p>
        </div>
      </div>
    </div>
